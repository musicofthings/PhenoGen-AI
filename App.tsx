...
function App() {
  ...
  const bakeStaticAssets = async () => {
    ...
    addLog("Connecting to Nano Banana 3 Pro Engine...");
    ...
    for (let i = 0; i < SLIDES.length; i++) {
      ...
      if (existing) {
        addLog(`Artifact ${slide.id} found in local persistent store. Skipping.`);
        setProvisionProgress(i + 1);
        continue;
      }

      try {
        addLog(`Synthesizing high-fidelity clinical visual for ID:${slide.id}...`);
        ...
        if (part?.inlineData) {
          ...
          addLog(`Success: Artifact ${slide.id} deployed to IndexedDB.`);
        }
      } catch (e) {
        addLog(`CRITICAL: Provisioning failed for slide ${slide.id}. Retrying next.`);
        console.error(e);
      }
      setProvisionProgress(i + 1);
    }
    
    addLog("Build Complete. Unlocking Static Presentation Deck.");
    setTimeout(() => {
  ...
  const resetBuild = async () => {
    if (confirm("Reset Build? This will delete all 20 pre-rendered clinical assets and require a full re-provisioning.")) {
      ...
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 font-mono text-blue-400">
        <div className="max-w-2xl w-full space-y-10">
          <div className="flex items-center space-x-6">
            <div className="w-20 h-20 bg-blue-600 rounded-3xl flex items-center justify-center text-white text-4xl font-black shadow-[0_0_30px_rgba(37,99,235,0.4)] animate-pulse-soft">Φ</div>
            <div>
          ...
          <div className="bg-[#1e293b] rounded-3xl p-8 border border-slate-700/50 shadow-2xl relative overflow-hidden group">
            ...
            {!isProvisioning ? (
              <div className="py-8 text-center space-y-8">
                <p className="text-slate-400 text-sm leading-relaxed max-w-sm mx-auto">
                  To ensure a zero-latency presentation experience on GitHub Pages, we must pre-render all 20 clinical visualizations.
                </p>
                <button 
              ...
              <div className="space-y-6">
                 <div className="h-64 overflow-y-auto custom-scrollbar pr-4 space-y-2 text-[10px]">
                    {buildLogs.map((log, i) => (
          ...
          <div className="flex justify-between items-center text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">
            <span>Target: static.github.io/phenogen-deck</span>
    ...
    <div className="min-h-screen flex flex-col bg-[#fcfdfe]">
      <header className="px-8 py-6 border-b border-slate-100 flex justify-between items-center sticky top-0 z-[100] bg-white/80 backdrop-blur-xl">
        <div className="flex items-center space-x-5">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tighter font-medical leading-none">PhenoGen</h1>
            <div className="flex items-center space-x-2 mt-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.25em]">Deployed Static Deck</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-6">
          <button 
            ...
            className="group flex items-center space-x-3 px-6 py-3 rounded-2xl bg-slate-50 text-slate-600 hover:bg-slate-900 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest border border-slate-100"
          >
            {viewMode === 'present' ? (
              <><LayoutGrid className="w-4 h-4" /><span>Board Overview</span></>
            ) : (
              <><Monitor className="w-4 h-4" /><span>Live Session</span></>
            )}
          </button>
          ...
          <div className="flex items-center space-x-4">
            <button 
              ...
              className="p-3 rounded-2xl border border-slate-200 hover:bg-white hover:shadow-lg disabled:opacity-20 disabled:cursor-not-allowed transition-all active:scale-95"
            >
      ...
      <main className="flex-1 relative flex flex-col p-6 md:p-12 lg:px-24 overflow-y-auto">
        {viewMode === 'present' ? (
          <div className={`w-full max-w-[1400px] mx-auto min-h-0 transition-all duration-500 transform ${isTransitioning ? 'opacity-0 translate-y-8 scale-[0.98] blur-sm' : 'opacity-100 translate-y-0 scale-100 blur-0'}`}>
            <Slide slide={SLIDES[currentSlideIndex]} />
          </div>
        ) : (
          <div className="space-y-10">
            <div className="flex justify-between items-center max-w-[1400px] mx-auto px-4">
              <h2 className="text-2xl font-black text-slate-900 tracking-tight">Artifact Repository</h2>
              <button 
                onClick={resetBuild}
                className="text-[10px] font-black uppercase tracking-widest text-rose-500 bg-rose-50 px-4 py-2 rounded-xl hover:bg-rose-600 hover:text-white transition-all flex items-center space-x-2"
              >
                <span>Reset Build Database</span>
              </button>
            </div>
            <div className="w-full max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10 p-4">
              {SLIDES.map((slide, idx) => (
                <button
                  ...
                  className={`group relative text-left bg-white rounded-[2.5rem] border-2 overflow-hidden transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] hover:-translate-y-3 ${currentSlideIndex === idx ? 'border-blue-600 ring-8 ring-blue-50 shadow-2xl' : 'border-slate-100'}`}
                >
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="h-0.5 w-4 bg-blue-600 rounded-full"></span>
      ...
      {viewMode === 'present' && (
        <div className="px-8 pb-10 sticky bottom-0 bg-gradient-to-t from-[#fcfdfe] via-[#fcfdfe] to-transparent pt-10">
           <div 
             ref={scrollRef}
             className="flex space-x-5 overflow-x-auto pb-6 custom-scrollbar no-scrollbar max-w-[1400px] mx-auto"
           >
             {SLIDES.map((s, i) => (
               <button
                 key={s.id}
                 onClick={() => goToSlide(i)}
                 className={`flex-shrink-0 w-32 h-20 rounded-[1.25rem] border-2 transition-all flex flex-col items-center justify-center space-y-2 relative overflow-hidden ${currentSlideIndex === i ? 'border-blue-600 bg-white shadow-2xl shadow-blue-100 ring-4 ring-blue-50' : 'border-slate-100 bg-white/50 hover:border-slate-300'}`}
               >
